...
import { initResetButtons } from '@modules/closing-element';
...
export default class Select extends ClassToggler {
  ...
  superInit() {
    ...
    if (this.$el.select) return false;
  ...
  setTemplateTitle() {
    ...
    for (const $option of this.options.values()) {
      if ($option.classList.contains(this.config.classes.checked)) {
        title = `<span>${$option.dataset.text}</span>`;
      }
    }
  ...
  setCounterTitle() {
    ...
    this.$title.innerHTML = title;
    if (counter > 0) {
      this.$counter.style.display = null;
  ...
  renderTemplate() {
    ...
    if (children[0].children.length && this.config.tabs) {
      layoutHTML += this.config.templates.tabsLayout.call(this, children);
    }
  ...
  open(e) {
    ...
    this._dispatchEvent(this.config.events.open, this, this.$select);
  }
  ...
  close(e) {
    ...
    this._dispatchEvent(this.config.events.close, this, this.$select);
  }
...
const defaults = {
  ...
  isWide: false, // если true - список внутри select__content в 3 колонки
  titlePlaceholder: '', // подпись селекта, которая уезжает наверх при выбранном option'е (указывать вместо placeholder)
  tabs: false, // если true - все optgroup разобьются по табам (обязательно должны быть optgroup)
  tagsOutside: false, // если true то теги будут расположены вне инпута
  ...
  // Templates
  templates: {
    layout(html = '', tabButtons) {
      const button = this.config.templates.button.call(this);
      let search = '';
      const tabButtonsContainer = `
      <div class="tabs tabs--services" data-tabs="${this.$el.id}">${tabButtons}</div>
      `;
      ...
      return `
        ...
        <div class="select__content">
          <div class="select__container">
           ${this.config.tabs ? tabButtonsContainer : ''}
            ${this.config.search.inside ? search : ''}
        ...
        <div class="select__content">
          <div class="select__container">
           ${this.config.tabs ? tabButtonsContainer : ''}
            ...
            <div data-scrollbar class="select__list">
              <ul class="select__list-wrapper">
                ${html}
              </ul>
            </div>
          </div>
    ...
    tabButton(text, tabId) {
      return `
        <button class="button button-tab" data-tab="${tabId}" type="button">
          <span>${text}</span>
        </button>
      `;
    },
    tabsLayout(optgroupArray) {
      let tabContents = '';

      optgroupArray.forEach((optgroup, index) => {
        let optionsHTML = '';
      ...
      <div class="tabs-contents" data-tabs-contents="${this.$el.id}">
        ${tabContents}
      </div>
      `;
    },
    button() {
      ...
      const titlePlaceholder = this.config.titlePlaceholder
        ? `<span class="select__title-placeholder">${this.config.titlePlaceholder}</span>`
        : '';

      if (this.config.tagsOutside) {
        return `
        <button
          type="button"
        ...
        <div class="select__search">
          <div class="input-wrapper">
            <div class="input-container">
              ...
              <button class="button button--secondary button-icon input-control" data-reset-btn type="button">
                <svg>
